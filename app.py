...
def get_condition_note(cash, loan, area_group, condition, lines, household):
    ...
    return "입력하신 조건(" + ", ".join(notes) + ")에 따라 우선순위를 적용해 추천했습니다."
...
if submitted:
    ...
    if len(df_filtered) < 3:
        extra_pool = df[(df['실사용가격'] > budget_upper) & (df['실사용가격'] <= budget_cap)]
        df_filtered = pd.concat([df_filtered, extra_pool]).drop_duplicates(subset=['단지명', '전용면적'])
...
""")
    ...
    st.markdown("---")
    st.markdown("※ 위 추천은 사용자의 입력 조건과 시장 정보를 종합적으로 판단하여 제안드린 결과입니다.")
